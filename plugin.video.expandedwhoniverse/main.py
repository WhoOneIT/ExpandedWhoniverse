...
Serials_path = os.path.join(addon_path, 'resources', 'submenu3.csv')
Episodes_path = os.path.join(addon_path, 'resources', 'Episodes.csv')
Seasons_path = os.path.join(addon_path, 'resources', 'submenu2.csv')
...
def add_directory_item(name, query, iconimage=None, description=None, is_folder=True, runtime=None, episode_number=None, airdate=None, mode=None, url=None):
    ...
    if description:
        ...
        li.setInfo('video', {'title': name, 'plot': description, 'premiered': airdate, 'duration': f"{runtime} min", 'episode': episode_number, 'year': int(year) if year.isdigit() else None})
...
#List series numbers for each show by linking in with submenu2.csv
...
def main_menu():
    add_directory_item('All Content',
    ...
    add_directory_item('Bonus Features', 
                   ...
                   'Additional content from across the Whoniverse, including Trailers, Convention Footage and more - often varies in quality.',  
                   is_folder=True)
    xbmcplugin.endOfDirectory(addon_handle)
